...
本リポジトリは**パブリックリポジトリ**として公開されています(2026-07-18 アルファリリース)。

- **作業ブランチは `develop`**。エージェントによる実装・ドキュメント作業のコミットは、必ず `develop` ブランチに対して行うこと(`master` へ直接コミットしない)。
- **`master` はリリースブランチ**。公開に足る区切りで、userの判断により `develop` から取り込む。エージェントが独断で `master` へのマージや push を行わないこと。
- 作業開始時にカレントブランチが `develop` であることを確認する(`git branch --show-current`)。異なっていれば `develop` へ切り替えてから作業する。
- push は user の指示があったときのみ行う(コミットまでが既定の作業範囲)。
